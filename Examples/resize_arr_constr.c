...
int resize(vector_t* vector){
	...
	// To double capacity of array.
	// create new array(newData) and use it to find doubled malloc memory.
	...
	// because the data is already stored into newData.
	free(vector->data);
	vector->data = newData;
	return 1;
}
...
int push_back(vector_t* vector, int element){
	...
	// Do we have to resize our array.
